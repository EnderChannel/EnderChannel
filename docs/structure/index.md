...
layout: page
title: 我们的团队
description: EnderChannel团队成员列表
---
<script setup>
import {
  VPTeamPage,
...
const code = [
    ...
    name: 'EnderPhantomWing',
    title: '网络开发',
    desc: 'MCJPG官网维护 | AI项目负责人 | MC生电服腐竹',
    links: [
      { icon: 'github', link: 'https://github.com/EnderPhantomWing' },
    ]
  },
  {
...
<VPTeamPage>
  ...
  <VPTeamPageTitle>
    ...
    <template #lead>认识一下 EnderChannel 背后的成员, 主要折腾各种小玩意, 研究学习开源项目</template>
  </VPTeamPageTitle>
  <VPTeamPageSection>
    <template #title>管理部</template>
    <template #lead>EnderChannel 管理部 开发者帮助编写代码、审核项目, 他们使 EnderChannel 得以运行</template>
    <template #members>
      <VPTeamMembers size="small" :members="code" />
    </template>
  </VPTeamPageSection>
  <VPTeamPageSection>
    <template #title>成员部</template>
    <template #lead>EnderChannel 成员部成员</template>
    <template #members>
